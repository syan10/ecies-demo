...
secure_t * ecies_encrypt(char *key, unsigned char *data, size_t length) {
        ...
        else if ((block_length = EVP_CIPHER_block_size(ECIES_CIPHER)) == 0 || block_length > EVP_MAX_BLOCK_LENGTH || (envelope_length = EC_POINT_point2oct(EC_KEY_get0_group(ephemeral), EC_KEY_get0_public_key(ephemeral), POINT_CONVERSION_COMPRESSED, NULL, 0, NULL)) == 0) {
        ...
        else if (EC_POINT_point2oct(EC_KEY_get0_group(ephemeral), EC_KEY_get0_public_key(ephemeral), POINT_CONVERSION_COMPRESSED, secure_key_data(cryptex), envelope_length,                        NULL) != envelope_length) {
        ...
        if (HMAC_Init_ex(hmac, envelope_key + key_length, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(hmac, secure_body_data(cryptex), secure_body_length(cryptex))!= 1 || HMAC_Final(hmac, secure_mac_data(cryptex), &mac_length) != 1) {
...
unsigned char * ecies_decrypt(char *key, secure_t *cryptex, size_t *length) {
        ...
        // Make sure we are generating enough key material for the symmetric ciphers.
                ...
                printf("The key derivation method will not produce enough envelope key material for the chosen ciphers. {envelope = %i / required = %zu}", SHA512_DIGEST_LENGTH / 8,(key_length * 2) / 8);return NULL;
        }
        ...
        // At the moment we are generating the hash using encrypted data. At some point we may want to validate the original text instead.
        if (HMAC_Init_ex(hmac, envelope_key + key_length, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(hmac, secure_body_data(cryptex), secure_body_length(cryptex))!= 1 || HMAC_Final(hmac, md, &mac_length) != 1) {
        ...
        if (EVP_DecryptInit_ex(cipher, ECIES_CIPHER, NULL, envelope_key, iv) != 1 || EVP_CIPHER_CTX_set_padding(cipher, 0) != 1 || EVP_DecryptUpdate(cipher, block,&output_length, secure_body_data(cryptex), secure_body_length(cryptex)) != 1) {
